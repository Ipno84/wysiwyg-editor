...
import { getAuthorableStateFilters } from "@/editor/state/editor/selectors/get-authorable-state-filters";
...
const getSelectedAuthorableStateFilter = createSelector(
  ...
  (selectedAuthorableKey, authorableStateFilters) => {
    if (authorableStateFilters[selectedAuthorableKey])
      return authorableStateFilters[selectedAuthorableKey];
    return null;
  },
);
