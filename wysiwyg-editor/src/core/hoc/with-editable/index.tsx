...
import { getAuthorableStateByKey } from "@/editor/state/editor/selectors/get-authorable-state-by-key";
import { isSelectedAuthorableKey as isSelectedAuthorableKeySelector } from "@/editor/state/editor/selectors/is-selected-authorable-key";
import type { IEditorState } from "@/editor/state/editor/typings";

const withEditable = <T extends Record<string, any>>(
  ...
  const WithEditableComponent = (props: T) => {
    ...
    const isSelectedAuthorableKey = useEditorStore((state: IEditorState) =>
      isSelectedAuthorableKeySelector(state, props.uuid),
    );

    const authorableState = useEditorStore((state: IEditorState) =>
      getAuthorableStateByKey(state, props.uuid),
    );
